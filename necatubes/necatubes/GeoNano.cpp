//===========================================================================
// GeoNano.cpp
// NanoCompositesÎ¢½á¹¹¼¸ºÎ½¨Ä£³ÉÔ±º¯Êı
...
int GeoNano::Geometric_modeling(ifstream &infile, const int &samples_count)
{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------------
	//Éú³ÉÄÉÃ×¹ÜÍøÂç
	vector<struct elliparam> ellips;					//¶¨ÒåÄÉÃ×¹ÜÍÅ´ØÍÖÇòĞÅÏ¢
	vector<vector<Point_3D> > cnts_points; //¶¨ÒåÈıÎ¬µãÏòÁ¿(ÓÃÓÚ¼ÇÂ¼ÄÉÃ×¹ÜÍøÂç)
	...
	else	{ hout << "ÄÉÃ×¹ÜÉú³É×¼Ôò¼È²»ÊÇÌå»ı(vol)Ò²²»ÊÇÖØÁ¿(wt)£¡ Çë¼ì²é£¡" << endl; return 0; }
	...
	//-----------------------------------------------------------------------------------------------------------------------------------------
	//Êä³öÄÉÃ×¹ÜÍøÂç¼¸ºÎ¹¹Ôì(ÏßTecplot)
	//if(Export_cnt_networks_threads(cell_geo, cnts_points)==0) return 0;
    
	//-----------------------------------------------------------------------------------------------------------------------------------------
	//Êä³öÄÉÃ×¹ÜÍøÂç¼¸ºÎ(Template for Sofiane)
    //	if(Export_cnt_fibers(cnts_points)==0) return 0;
    
	//-----------------------------------------------------------------------------------------------------------------------------------------
	//Êä³öÄÉÃ×¹ÜÍøÂç¼¸ºÎ¹¹Ôì(ËÄÃæÌåÍø¸ñTecplot) //×¢ÊÍÔÚRVE±íÃæ´¦»¹Ã»ÓĞ¿¼ÂÇ±íÃæÒªÇĞµôÄÉÃ×¹ÜµÄÒ»²¿·Ö
    //if(Export_cnt_networks_meshes(cell_geo, cnts_points, ellips)==0) return 0;
    
	//-----------------------------------------------------------------------------------------------------------------------------------------
	//Êä³öÄÉÃ×ÏßÒÔ¼°Í¿²ã¼¸ºÎ¹¹Ôì(ËÄÃæÌåÍø¸ñ(Í¿²ã)ºÍÄÉÃ×ÏßTecplot)
    //	if(Export_cnt_threads_coating_mesh(cell_geo, cnts_points)==0) return 0;
    
	//-----------------------------------------------------------------------------------------------------------------------------------------
	//Êä³öÄÉÃ×¹ÜÒÔ¼°Í¿²ã¼¸ºÎ¹¹Ôì(ËÄÃæÌåµ¥Ôª(ÄÉÃ×¹Ü)ÁùÃæÌåµ¥Ôª(Í¿²ã)Tecplot)(Í¿²ãºñ¶ÈÔÚ´Ëº¯ÊıÖĞÉèÖÃ)
    //	if(Export_cnt_coating_meshes(cell_geo, cnts_points)==0) return 0;
    
	//-----------------------------------------------------------------------------------------------------------------------------------------
	//¼ÆËãÄÉÃ×¹ÜÍÅ´ØÍÖÇòÖĞÄÉÃ×¹ÜµÄ±ÈÖØºÍÌå»ı·ÖÊı
	//if(Estimate_cnt_cluster_parameters(cell_geo, cnts_points, ellips, cnts_geo, clust_geo)==0) return 0;
    
	//-----------------------------------------------------------------------------------------------------------------------------------------
	//¼ÇÂ¼ÄÉÃ×¹ÜµãĞÅÏ¢(PointµÄflag==0±íÊ¾ÄÉÃ×¹ÜµÄÆğÊ¼µã; flag>0±íÊ¾ÄÉÃ×¹ÜÄÚµÄµã, °´ĞòÒÀ´Î±àºÅ)
    //================================================================
...
int GeoNano::Import_geometric_data(ifstream &infile, struct RVE_Geo &cell_geo, struct Region_Geo &overlap_regions, struct Region_Geo &cnt_regions, struct CNT_Geo &cnts_geo, struct Clust_Geo &clust_geo, const int &samples_count, vector<vector<long int> > &sectioned_domain, vector<vector<int> > &sectioned_domain_cnt, double &d_vdw, double &cutoff)const
{
	...
	cell_geo.volume = cell_geo.len_x*cell_geo.wid_y*cell_geo.hei_z;		//RVEÌå»ı
	...
	//-----------------------------------------------------------------------------------------------------------------------------------------
	//¶ÁÈëÄÉÃ×¹ÜÃ¿²½Éú³¤Ê±½Ç¶È·ş´ÓÕıÌ¬·Ö²¼µÄ×î´óÈ¡Öµ·¶Î§
	...
	else if(cnts_geo.criterion=="wt"||cnts_geo.criterion=="nwt")
	{
		...
		if(cnts_geo.weight_fraction>1||cnts_geo.weight_fraction<0){ hout << "ÄÉÃ×¹ÜÔÚRVEÖĞËùÕ¼±ÈÖØ·ÖÊıÊäÈë´íÎó£¡ Çë¼ì²é£¡" << endl; return 0; }
		hout << "    »ù´¡±ÈÖØÖµÊÇ£º " << cnts_geo.weight_fraction << endl;
        
		int mode_accum;
		istr_cnt_vol >> mode_accum;
		if(mode_accum<0&&mode_accum>2){ hout << "ÖØÁ¿·ÖÊıÀÛ¼Æ·½Ê½´íÎó£¡ Çë¼ì²é£¡" << endl; return 0; }
		...
		hout << "    Êµ¼Ê±ÈÖØÖµÊÇ£º " << cnts_geo.weight_fraction << endl;
        
		istr_cnt_vol >> cnts_geo.linear_density;  //¶ÁÈëÄÉÃ×¹ÜµÄÏßÃÜ¶È
		if(cnts_geo.linear_density<0){ hout << "ÄÉÃ×¹ÜµÄÏßÃÜ¶ÈÊäÈë´íÎó£¡ Çë¼ì²é£¡" << endl; return 0; }
		istr_cnt_vol >> cell_geo.density;				//¶ÁÈë»ùÌå(polymer)µÄÃÜ¶È£¬ÕâÀïÓÉÓÚºöÂÔÄÉÃ×¹ÜµÄÌå»ı£¬ËùÒÔ»ùÌåµÄÃÜ¶È½üËÆÒ²ÊÇµ¥°ûµÄÃÜ¶È
		if(cell_geo.density<0){ hout << "»ùÌåµÄÌåÃÜ¶ÈÊäÈë´íÎó£¡ Çë¼ì²é£¡" << endl; return 0; }
		if(cnts_geo.linear_density>=cell_geo.density){ hout << "ÄÉÃ×¹ÜµÄÏßÃÜ¶È»òÕß»ùÌåµÄÌåÃÜ¶ÈÊäÈë´íÎó£¡ Çë¼ì²é£¡" << endl; return 0; }
        
		//Êµ¼ÊÄÉÃ×¹Ü±ÈÖØ(Êµ¼ÊÉÏÄÉÃ×¹ÜµÄÏßÃÜ¶ÈÊÇ¿ÉÒÔÔÚÒ»¸ö·¶Î§ÄÚ±ä»¯µÄ£¬ÈçÍ¬°ë¾¶)
		cnts_geo.real_weight = cnts_geo.weight_fraction*cell_geo.volume*cell_geo.density;
	}
	else { hout << "ÄÉÃ×¹ÜÆÀ¼Û×¼Ôò¼È²»ÊÇÌå»ı(vol)Ò²²»ÊÇÖØÁ¿(wt)£¬ÊäÈë´íÎó£¡ Çë¼ì²é£¡" << endl; return 0; }
	
	//-----------------------------------------------------------------------------------------------------------------------------------------
	//¶ÁÈëÄÉÃ×¹ÜÍÅ´ØÍÖÇò²ÎÊı
	istringstream istr_clust_para(Get_Line(infile));
	istr_clust_para >> clust_geo.vol_fra_criterion;
	if(clust_geo.vol_fra_criterion>1||clust_geo.vol_fra_criterion<0){ hout << "ÄÉÃ×¹ÜÍÅ´ØÍÖÇòÔÚRVEÖĞËùÕ¼Ìå»ı·ÖÊıÊäÈë´íÎó£¡ Çë¼ì²é£¡" << endl; return 0; }
    clust_geo.vol_fra_criterion = clust_geo.vol_fra_criterion*pow(2.0, samples_count-1);
	if(clust_geo.vol_fra_criterion!=0)  //²»Éú³ÉÄÉÃ×¹ÜÍÅ´ØÊ±£¬Ìø¹ı¶ÁÈëÍÅ´ØÍÖÇòĞÅÏ¢
	{
		istr_clust_para >> clust_geo.amin >> clust_geo.amax;
		if(clust_geo.amin<0||clust_geo.amax<0||clust_geo.amin>clust_geo.amax){ hout << "ÄÉÃ×¹ÜÍÅ´ØÍÖÇòµÄ³¤ÖáÈ¡Öµ·¶Î§ÊäÈë´íÎó£¡ Çë¼ì²é£¡" << clust_geo.amin << "  " << clust_geo.amax << endl; return 0; }
		...
		if(clust_geo.bmin<0||clust_geo.bmin>clust_geo.amin||clust_geo.cmin<0||clust_geo.cmin>clust_geo.bmin||
           clust_geo.cmin>clust_geo.amin){ hout << "ÄÉÃ×¹ÜÍÅ´ØÍÖÇòµÄÖĞÖá»ò¶ÌÖáÈ¡Öµ·¶Î§ÊäÈë´íÎó£¡ Çë¼ì²é£¡" << endl; return 0; }
		istr_clust_para >> clust_geo.growth_probability;
		if(clust_geo.growth_probability<0||clust_geo.growth_probability>1){ hout << "ÄÉÃ×¹ÜÔÚÍÅ´ØÍÖÇòÖĞÉú³¤µÄ¸ÅÂÊÊäÈë´íÎó(<0»òÕß>1)£¡ Çë¼ì²é£¡" << endl; return 0; }
		istr_clust_para >> clust_geo.wt_fra_cluster;
		if(clust_geo.wt_fra_cluster<0||clust_geo.wt_fra_cluster>1) { hout << "ÄÉÃ×¹ÜÔÚÍÅ´ØÍÖÇòÖĞÄÉÃ×¹ÜµÄÖØÁ¿ÊäÈë´íÎó(<0»òÕß>1)£¡ Çë¼ì²é£¡" << endl; return 0; }
...
int GeoNano::New_generate_nanotube_networks(const struct RVE_Geo &cell_geo, const struct CNT_Geo &cnts_geo, vector<vector<Point_3D> > &cnts_points, struct Clust_Geo &clust_geo, vector<struct elliparam> &ellips)
{
	...
	//---------------------------------------------------------------------------
	//Éú³ÉÄÉÃ×¹ÜÍÅ´ØÍÖÇòĞÅÏ¢£¨ÍÖÇò¶¼ÔÚµ¥°ûÄÚ²¿²¢Ïà»¥·ÖÀë£©
	if(clust_geo.vol_fra_criterion>0.0)
	{
		...
		//×îºóÒ»Î»Êµ²Î£º0±íÊ¾²»Êä³ö£»1±íÊ¾Ö»Êä³öÄÉÃ×¹ÜÍÅ´ØÍÖÇòÊı¾İ£»2±íÊ¾Êä³öÍÅ´ØÍÖÇòÊı¾İ¼°ÍÖÇòÃæÍø¸ñ
		if(Get_ellip_clusters(cell_geo, clust_geo, seed_ellip_poi, seed_ellip_axis, seed_ellip_angle, ellips, 1)==0) return 0;
        //		if(Get_specific_sphere_clusters(cell_geo, clust_geo, seed_ellip_poi, seed_ellip_axis, seed_ellip_angle, ellips, 1)==0) return 0;  //Éú³ÉÌØ¶¨Î»ÖÃµÄÔ²ÇòÍÅ´Ø
	}
    
	//---------------------------------------------------------------------------
	//¼ÆËãÄÉÃ×¹ÜÔÚÃ¿¸öÍÅ´Ø¿ÅÁ£ÖĞµÄ³¤¶ÈÒÔ¼°ÆäÓàËæ»ú·Ö²¼µÄÄÉÃ×¹Ü³¤¶È
	...
	if(cnt_weight_random<=0) { hout << "´íÎó! ÄÉÃ×¹ÜÔÚµ¥°ûÖĞµÄ×ÜÖØÁ¿Ğ¡ÓÚÄÉÃ×¹ÜÔÚÍÅ´Ø¿ÅÁ£ÖĞÖØÁ¿µÄ×ÜºÍ£¡ Çë¼ì²é£¡" << endl;
        hout << "if(cnt_weight_random<=0) \n" << endl;return 0; }
    
	//---------------------------------------------------------------------------
	//¹¹Ôì¸÷Ëæ»úÖÖ×Ó
	int seed_cnt_origin = rand()%MAX_INT;     //Èç¹ûRAND_MAX==2^15, ÄÇÃ´rand()È¡[0, RAND_MAX]£¨×¢£º16Î»»ú£©
	int seed_cnt_length = rand()%MAX_INT;		//Èç¹ûRAND_MAX==2^31, ÄÇÃ´rand()È¡[0, MAX_INT]£¨×¢£º32Î»»ú£©, ¶ÔÈ¡Ëæ»úÊıÀ´Ëµ [0, MAX_INT]·¶Î§×ã¹»´ó
	...
	while(wt_sum<cnt_weight_random&&one_step_weight<cnt_weight_random)
	{
		...
		//---------------------------------------------------------------------------
		//ÄÉÃ×¹ÜÃ¿Éú³¤Ò»²½ËùÔö¼ÓµÄÌå»ıÏµÊı(ÕâÀïÃ»ÓĞ¿¼ÂÇÄÉÃ×ÏßÍøÂçÌå»ıÏàµşµÄÇé¿ö)
		const double step_vol_para = PI*cnt_rad*cnt_rad;
		//---------------------------------------------------------------------------
		//ÄÉÃ×¹ÜÃ¿Éú³¤Ò»²½ËùÔö¼ÓµÄÖØÁ¿ÏµÊı(µ±¿¼ÂÇÄÉÃ×´ÖÏ¸²»Í¬Ê±£¬ÏßÃÜ¶ÈÒ²ÊÇÔÚÒ»¸ö·¶Î§ÄÚ¿É±äµÄ£¬Ã¿¸ùÄÉÃ×¹Ü¿ÉÄÜ²»Í¬)
		...
		{
			//°´ÓëZÖáÕıÏòÒ»ÖÂµÄ·½ÏòÎªÖĞĞÄ, ¾¶ÏòÕıÌ¬·Ö²¼sita = fabs[(-omega,+omega)], Í¨³£omega<=PI/2, Î³Ïò¾ùÔÈ·Ö²¼pha=(0,2PI)
			//Ëæ»úÑ¡È¡·½Ïò
			...
			if(ei0>=0)
			{
				//¼ÆËãÄÉÃ×¹ÜÏß¶ÎÓëÍÖÇòÃæµÄ½»µã(ÕÒ²»µ½½»µã±¨´í)
				Point_3D intersect_point(0, 0 ,0);
				if(Get_intersection_cnt_ellipsoid(ellips[ei0], new_cnt.back(), cnt_poi, intersect_point)==0) { hout << "´íÎó£¡Ã»ÓĞÕÒµ½ÄÉÃ×¹ÜÏßºÍÍÖÇòÃæµÄ½»µã£¬Çë¼ì²é£¡" << endl; return 0; }
				if(intersect_point.flag==-1)  { hout << new_cnt_size << "  " << "´íÎó£¡ÄÉÃ×¹ÜÉÏµÄµãÓëÍÅ´ØÍÖÇòÃæÉÏµÄ½»µã¾àÀë¹ı½ü£¡" << endl; return 0; }
				
				new_cnt.push_back(intersect_point);		//´æ´¢½Úµã
			...
			//---------------------------------------------------------------------------
			//Èç¹ûÉú³¤µãÔÚµ¥°ûÒÔÍâ£¬Æ½ÒÆÄÉÃ×¹ÜÉú³¤
			if(Judge_cell_including_point(cell_geo, cnt_poi)==0)
			{
				//¼ÆËãÕâÁ½¸ö¶ËµãÓëµ¥°û½çÃæµÄËùÓĞ½»µã(ÔÚÁ½¸ö¶ËµãµÄÁ¬ÏßÖ®¼ä£¬²ÎÊı0<t<1)£¬²¢°´²ÎÊıt´ÓĞ¡µ½´óÅÅÁĞÕâĞ©½»µã
				vector<Point_3D> ipoi_vec;  //½»µãĞòÁĞ
				...
				for(int j=0; j<(int)ipoi_vec.size(); j++)
				{
					...
					if(j==(int)ipoi_vec.size()-1) cnt_poi = inter_point[1];  //¸üĞÂÎ»ÖÃ
				...
				if(ei1>=0)
				{
					//¼ÆËãÄÉÃ×¹ÜÏß¶ÎÓëÍÖÇòÃæµÄ½»µã(ÕÒ²»µ½½»µã±¨´í)
					Point_3D intersect_point(0, 0 ,0);
					if(Get_intersection_cnt_ellipsoid(ellips[ei1], new_cnt[(int)new_cnt.size()-2], new_cnt.back(), intersect_point)==0) { hout << "´íÎó£¡Ã»ÓĞÕÒµ½ÄÉÃ×¹ÜÏßºÍÍÖÇòÃæµÄ½»µã£¬Çë¼ì²é£¡" << endl; return 0; }
					if(intersect_point.flag==-1)  { hout << new_cnt_size << "  " << "´íÎó£¡ÄÉÃ×¹ÜÉÏµÄµãÓëÍÅ´ØÍÖÇòÃæÉÏµÄ½»µã¾àÀë¹ı½ü£¡" << endl; return 0; }
			...
			//---------------------------------------------------------------------------
			//ÅĞ¶Ï×ÜÌå»ı»òÕß×ÜÖØÁ¿
			if(wt_sum >= cnt_weight_random) break;			//ÖØÁ¿Âú×ã, Ìø³ö
		}
		...
		if((int)new_cnt.size()==1)
		{
			...
			hout << "Éú³ÉËæ»ú·Ö²¼²¿·ÖµÄÄÉÃ×¹ÜÊı¾İÓĞÎó£¡ Çë¼ì²é£¡" << endl;
			return 0;
		}
		...
		{
			cnt_temp.push_back(new_cnt[i]); //Ê×ÏÈ²åÈë½Úµã
			//ÅĞ¶ÏÊÇ·ñÊÇ½áÊø
			if(i==(int)new_cnt.size()-1||new_cnt[i+1].flag==0) //Ä©Î²µã½áÊø»òÕß×ª»»µ½ÁíÒ»ÌõÄÉÃ×¹Ü
			{
				if((int)cnt_temp.size()>1)						//µÈÓÚ1µÄÇé¿öËµÃ÷Ö»ÓĞÒ»¸öÆğÊ¼µã£¬ÕâÖÖÇé¿ö²»¹¹³ÉÄÉÃ×¹ÜÏß¶Î
				{
					cnts_points.push_back(cnt_temp);		//¼ÇÂ¼ÄÉÃ×¹ÜĞÅÏ¢
					cnts_radius.push_back(cnt_rad);			//¼ÇÂ¼ÄÉÃ×¹Ü°ë¾¶
				}
				cnt_temp.clear();									//Çå¿ÕÁÙÊ±ÄÉÃ×¹Ü
			}
		}
	}
	//¼ÇÂ¼ÄÉÃ×¹ÜÌå»ıºÍÖØÁ¿
	...
	//----------------------------------------------------------------------------------------------------------------------------------------------------------------------
	//----------------------------------------------------------------------------------------------------------------------------------------------------------------------
	//Ñ­»·ÄÉÃ×¹ÜÍÅ´ØÍÖÇò, ÒÀ´ÎÉú³ÉÍÅ´ØÍÖÇòÄÚÄÉÃ×¹Ü
	for(int i=0; i<(int)ellips.size(); i++)
	{
		...
		while(wt_sum < cnt_weight_cluster[i])
		{
			...
			//---------------------------------------------------------------------------
			//·Ö±ğÔÚÒÔÍÖÇòµÄ³¤ÖĞ¶ÌÖá2±¶Îª³¤¿í¸ßµÄÁ¢·½ÌåÉÏ°´¾ùÔÈ·Ö²¼Ñ¡È¡ÄÉÃ×¹ÜµÄÆğÊ¼µã
			//²¢ÅĞ¶Ï´ËµãÊÇ·ñÔÚÍÖÇòÄÚ£¬Èç¹ûÔÚÍÖÇòÄÚ£¬ÔÙ±ä»»µ½ÍÖÇò×ø±êÏµÏÂ
			Point_3D cnt_poi;
			...
			//---------------------------------------------------------------------------
			//ÄÉÃ×¹ÜÃ¿Éú³¤Ò»²½ËùÔö¼ÓµÄÌå»ı(ÕâÀïÃ»ÓĞ¿¼ÂÇÄÉÃ×ÏßÍøÂçÌå»ıÏàµşµÄÇé¿ö)
			const double step_vol_para = PI*cnt_rad*cnt_rad;
			//---------------------------------------------------------------------------
			//ÄÉÃ×¹ÜÃ¿Éú³¤Ò»²½ËùÔö¼ÓµÄÖØÁ¿(µ±¿¼ÂÇÄÉÃ×´ÖÏ¸²»Í¬Ê±£¬ÏßÃÜ¶ÈÒ²ÊÇÔÚÒ»¸ö·¶Î§ÄÚ¿É±äµÄ£¬Ã¿¸ùÄÉÃ×¹Ü¿ÉÄÜ²»Í¬)
			const double step_wei_para = cnts_geo.linear_density;
			...
			for(int j=0; j<step_num; j++)
			{
				...
				//---------------------------------------------------------------------------
				//Èç¹ûÄÉÃ×¹ÜÉú³¤´©Ô½ÍÅ´Ø½çÃæ£¬½«´ËÄÉÃ×¹Ü°´Ò»¶¨µÄ¸ÅÂÊÕÛ»Ø
				while(Cnt_go_through_a_cluster(clust_geo.growth_probability, ellips[i], seed_growth_probability, new_cnt.back(), cnt_poi)) //ĞèÒªÕÛ»Ø
				{
					//¼ÆËãÄÉÃ×¹ÜÏß¶ÎÓëÍÖÇòÃæµÄ½»µã(ÕÒ²»µ½½»µã±¨´í)
					Point_3D intersect_point(0, 0 ,0);
					if(Get_intersection_cnt_ellipsoid(ellips[i], new_cnt.back(), cnt_poi, intersect_point)==0) { hout << "´íÎó£¡Ã»ÓĞÕÒµ½ÄÉÃ×¹ÜÏßºÍÍÖÇòÃæµÄ½»µã£¬Çë¼ì²é£¡" << endl; return 0; }
					if(intersect_point.flag==-1)  break;  //ÅĞ¶ÏÌØÊâÇé¿ö£ºÕâÌõÏß¶ÎµÄÒ»¸öµãÔÚÍÖÇòÃæÉÏ£¬ÕûÌåÊµ¼ÊÉÏÔÚÍÖÇòÍâ²¿
                    
					//ÒÆ¶¯ÄÉÃ×¹ÜÏß¶ÎÔÚÍÖÇòÃæÍâ¶Ëµãµ½ÍÖÇòÃæÄÚµÄ¶Ô³Æµã£¨ÒÔÇĞÃæÎª¶Ô³ÆÃæ£©×¢Òâ£º¾­¹ıÕâÒ»²½cnt_poiµÄÎ»ÖÃÒÑ¾­·¢ÉúÁË±ä»¯
					if(Move_cnt_point_symmetry_point(ellips[i], intersect_point, cnt_poi)==0) { hout << "´íÎó£¡Ã»ÓĞÕÒµ½ÄÉÃ×¹ÜÏßºÍÍÖÇòÃæµÄ½»µã£¬Çë¼ì²é£¡" << endl; return 0; }
                    
					//¿¼ÂÇÈıµã¼Ğ½Ç
					if(Cos_angle_in_three_points(new_cnt.back(), intersect_point, cnt_poi)<cos(PI/6)) //¼Ğ½Ç´óÓÚ30¶È
					{
						//²åÈë½»µãµ½ÄÉÃ×¹ÜĞòÁĞ£¨½»µãµÄflagÖµÔÚ¼ÆËãÊ±ÒÑ¾­¸³Óè£©
				...
				//---------------------------------------------------------------------------
				//ÅĞ¶Ï×ÜÌå»ı»òÕß×ÜÖØÁ¿
				if(wt_sum >= cnt_weight_cluster[i]) break;			//ÖØÁ¿Âú×ã, Ìø³ö
			}
			...
			if((int)new_cnt.size()==1)
			{
				hout << "Éú³ÉÍÅ´ØÍÖÇò" << i << "ÄÚ²¿µÄÄÉÃ×¹ÜÊı¾İÓĞÎó£¡ Çë¼ì²é£¡" << endl;
				return 0;
			}
			...
			{
				cnt_temp.push_back(new_cnt[j]); //Ê×ÏÈ²åÈë½Úµã
				//ÅĞ¶ÏÊÇ·ñÊÇ½áÊø
				if(j==(int)new_cnt.size()-1||new_cnt[j+1].flag==0) //Ä©Î²µã½áÊø»òÕß×ª»»µ½ÁíÒ»ÌõÄÉÃ×¹Ü
				{
					if((int)cnt_temp.size()>1)						//µÈÓÚ1µÄÇé¿öËµÃ÷Ö»ÓĞÒ»¸öÆğÊ¼µã£¬ÕâÖÖÇé¿ö²»¹¹³ÉÄÉÃ×¹ÜÏß¶Î
					{
						cnts_points.push_back(cnt_temp);		//¼ÇÂ¼ÄÉÃ×¹ÜĞÅÏ¢
	...
	hout << "    µ¥°ûÖĞÄÉÃ×¹ÜµÄ×ÜÌå»ı£º" << vol_total << endl;
	hout << "    µ¥°ûÖĞÄÉÃ×¹ÜµÄ×ÜÖØÁ¿£º" << wt_sum << endl;
...
int GeoNano::Generate_nanotube_networks(const struct RVE_Geo &cell_geo, const struct CNT_Geo &cnts_geo, vector<vector<Point_3D> > &cnts_points, struct Clust_Geo &clust_geo, vector<struct elliparam> &ellips)
{
	...
	double vol_sum = 0;  //¼ÇÂ¼ÄÉÃ×¹ÜÏßËùÕ¼Ìå»ıºÍ
	double wt_sum = 0;   //¼ÇÂ¼ÄÉÃ×¹ÜÏßËùÕ¼ÖØÁ¿ºÍ
	int cnt_seed_count =0; //¼ÇÂ¼ÈöÁË¶àÉÙ´ÎÄÉÃ×¹ÜÖÖ×Ó£¨ÆäÖĞ»áÓĞÉú³¤Ê§°ÜµÄ£¬µ«ÕâÁ£ÖÖ×ÓÈÔÔÚ£©
          ...
          (cnts_geo.criterion == "wt"&&wt_sum < cnts_geo.real_weight))
	{
		...
		new_cnt.push_back(cnt_poi);	//´æ´¢½Úµã
		cnt_seed_count++;					//¼ÇÂ¼Ñ¡ÖÖ³É¹¦Ò»´Î
		//if(cnt_seed_count>100000) { hout << "ÄÉÃ×¹ÜÈöÖÖ³¬¹ıÊ®Íò´Î£¬ÈÔÃ»ÓĞ´ïµ½ÒªÇóµÄÌå»ı·ÖÊı£¡ Çë¼ì²é£¡" << endl; return 0; }

		//---------------------------------------------------------------------------
		//ÄÉÃ×¹ÜÉú³¤
		int ellip_num = -1; //µ±¿¼ÂÇÍÖÇòÍÅ´ØÊ±£¬ÓÃÓÚ±ê¼ÇÄÉÃ×¹Ü´©³öµÄÍÖÇò±àºÅ£¬µ«ÄÉÃ×¹ÜÃ»ÓĞ¿ªÊ¼Éú³¤Ê±²»»á´©³öÈÎºÎÍÖÇò½çÃæ£»µ±²»¿¼ÂÇÍÖÇòÍÅ´ØÊ±£¬´Ë±äÁ¿Ã»ÓĞÓÃµ½
		for(int i=0; i<step_num; i++)
		{
			//°´ÓëZÖáÕıÏòÒ»ÖÂµÄ·½ÏòÎªÖĞĞÄ, ¾¶ÏòÕıÌ¬·Ö²¼sita = fabs[(-omega,+omega)], Í¨³£omega<=PI/2, Î³Ïò¾ùÔÈ·Ö²¼pha=(0,2PI)
			//Ëæ»úÑ¡È¡·½Ïò
			...
			if(ellips.size()>0)
			{
				while(Cnt_cross_cluster_surface(clust_geo.growth_probability, ellips, ellip_num, seed_growth_probability, new_cnt.back(), cnt_poi)) //ĞèÒªÕÛ»Ø
				{
					if(ellip_num<0||ellip_num>=(int)ellips.size()) { hout << "´íÎó£¡¼ÇÂ¼µÄÍÅ´ØÍÖÇò±àºÅ²»ÔÚÍÖÇòÏòÁ¿±àºÅ·¶Î§ÄÚ£¬Çë¼ì²é£¡" << endl; return 0; }
					//¼ÆËãÄÉÃ×¹ÜÏß¶ÎÓëÍÖÇòÃæµÄ½»µã(ÕÒ²»µ½½»µã±¨´í)
					Point_3D intersect_point(0, 0 ,0);
					if(Get_intersection_cnt_ellipsoid(ellips[ellip_num], new_cnt.back(), cnt_poi, intersect_point)==0) { hout << "´íÎó£¡Ã»ÓĞÕÒµ½ÄÉÃ×¹ÜÏßºÍÍÖÇòÃæµÄ½»µã£¬Çë¼ì²é£¡" << endl; return 0; }
					if(intersect_point.flag==-1)  break;  //ÅĞ¶ÏÌØÊâÇé¿ö£ºÕâÌõÏß¶ÎµÄÒ»¸öµãÔÚÍÖÇòÃæÉÏ£¬ÕûÌåÊµ¼ÊÉÏÔÚÍÖÇòÍâ²¿
                    
					//ÒÆ¶¯ÄÉÃ×¹ÜÏß¶ÎÔÚÍÖÇòÃæÍâ¶Ëµãµ½ÍÖÇòÃæÄÚµÄ¶Ô³Æµã£¨ÒÔÇĞÃæÎª¶Ô³ÆÃæ£©×¢Òâ£º¾­¹ıÕâÒ»²½cnt_poiµÄÎ»ÖÃÒÑ¾­·¢ÉúÁË±ä»¯
					if(Move_cnt_point_symmetry_point(ellips[ellip_num], intersect_point, cnt_poi)==0) { hout << "´íÎó£¡Ã»ÓĞÕÒµ½ÄÉÃ×¹ÜÏßºÍÍÖÇòÃæµÄ½»µã£¬Çë¼ì²é£¡" << endl; return 0; }
			...
			//---------------------------------------------------------------------------
			//ÅĞ¶Ï×ÜÌå»ı»òÕß×ÜÖØÁ¿
			if(cnts_geo.criterion == "vol"&&vol_sum >= cnts_geo.real_volume) break;			//Ìå»ıÂú×ã, Ìø³ö
			else if(cnts_geo.criterion == "wt"&&wt_sum >= cnts_geo.real_weight) break;		//ÖØÁ¿Âú×ã, Ìø³ö
		}
        ...
        if (delete_flag) {
            ...
            if(new_cnt.size()<2)
            {
                ...
                //Increase the count of rejects
                rejects++;
                hout << "Éú³ÉµÄÄÉÃ×¹ÜÊı¾İÓĞÎó£¡ Çë¼ì²é£¡" << endl;
                //return 0;
            } else {
                ...
                cnts_points.push_back(new_cnt);		//¼ÇÂ¼ÄÉÃ×¹ÜĞÅÏ¢
	...
	{
		hout << "    Éú³ÉµÄÄÉÃ×¹ÜËùÕ¼Ìå»ı°Ù·Ö±ÈÔ¼Îª£º" << vol_sum/cell_geo.volume << endl;
	}
...
//---------------------------------------------------------------------------
//¼ÆËã¿Õ¼äÒ»µãÓëÍÖÇòµÄÎ»ÖÃ¹ØÏµ£¬²¢·µ»ØÖµ
...
int GeoNano::Get_ellip_clusters(const struct RVE_Geo &cell, struct Clust_Geo &clust_geo, int &seed_poi, int &seed_axis, int &seed_angle, vector<struct elliparam> &ellips, const int &export_mod)const
{
	...
	do
	{
		...
		//ÍÖÇò¾Å¸ö·½Ïò½Ç[(alpha1,beta1,gamma1),(alpha2,beta2,gamma2),(alpha3,beta3,gamma3)]·Ö±ğÊÇÍÖÇòÖá(a,b,c)Óë(x,y,z)ÈıÖáµÄ¼Ğ½Ç
		...
		ell_temp.alpha1	=	cos(alpha1);																//alpha1´Ó(0, PI)ÖĞÑ¡
		ell_temp.beta1	=	cos(beta1);																	//beta1´Ó(pi/2-r1)µ½(pi/2+r1)Ñ¡È¡
		seed_angle	= (2053*seed_angle + 13849)%MAX_INT;
		ell_temp.gamma1 = pow(-1.0, fmod(seed_angle, 2.0)+1.0)*sqrt(1.0-pow(ell_temp.alpha1,2)-pow(ell_temp.beta1,2));	 //¼ÆËãgammaµÄÖµµ«Ëæ»úÈ¡Õı¸º·½Ïò
		double alpha2 = 0;																						//alpha2´Ó(pi/2-r1)µ½(pi/2+r1)Ñ¡È¡
		if(alpha1>PI/2.0)
		{
			seed_angle	= (2053*seed_angle + 13849)%MAX_INT;
	...
	//---------------------------------------------------------------------
	//Êä³öÄÉÃ×¹ÜÍÅ´ØÍÖÇòÍø¸ñ
	if(export_mod==2)	Export_cluster_ellipsoids_mesh(cell, ellips);
    
	//---------------------------------------------------------------------
	//Êä³öÄÉÃ×¹ÜÍÅ´ØÍÖÇòÊı¾İ
	if(export_mod==1||export_mod==2)	Export_cluster_ellipsoids_data(ellips, clust_geo.real_volume_fraction);
    
	//¼ì²éÊä³öÄ£Ê½
	if(export_mod>2||export_mod<0) { hout <<  "Éú³ÉÄÉÃ×¹ÜÍÅ´ØÍÖÇò³ÌĞòÖĞÊä³öÄ£Ê½´íÎó£¡ Çë¼ì²é£¡" << endl; return 0; }
	else {	hout << "    ÄÉÃ×¹ÜÍÅ´ØÍÖÇòµÄ¸öÊıºÍÊµ¼ÊÌå»ı£º" << (int)ellips.size() << "  " << clust_geo.real_volume_fraction << endl; }
    
	return 1;
}
//---------------------------------------------------------------------------
//Éú³ÉÌØ¶¨Î»ÖÃµÄÔ²ÇòÍÅ´ØĞòÁĞ(±ÜÃâËæ»úÇé¿öÊ±´óÊıÁ¿ÍÖÇòÎŞ·¨Éú³É)
...
{
	int snum = 2;	//µ¥°ûÃ¿Ò»±ßÉÏÉú³ÉÔ²ÇòµÄÊıÁ¿
	double sd_x = 0.5*cell.len_x/snum;
	double sd_y = 0.5*cell.wid_y/snum;
	double sd_z = 0.5*cell.hei_z/snum;
	
	if(clust_geo.amin!=clust_geo.amax||clust_geo.bmin!=0||clust_geo.cmin!=0) { hout << "×¢Òâ£¡²»·ûºÏÉú³ÉÌØ¶¨Î»ÖÃÔ²ÇòÍÅ´ØĞòÁĞµÄÌõ¼ş£¬Çë¼ì²é£¡" << endl; return 0; }
    
	if(sd_x<=clust_geo.amin||sd_y<=clust_geo.amin||sd_z<=clust_geo.amin) { hout << "×¢Òâ£¡µ¥°ûÓĞÒ»±ßÉÏÉú³ÉÔ²ÇòµÄÊıÁ¿¹ı¶à£¬Çë¼ì²é" << endl; return 0; }
	...
	{
		hout << "×¢Òâ£¡Éú³ÉÔ²ÇòÍÅ´ØĞòÁĞµÄ×ÜÌå»ıÊÇ" << clust_geo.real_volume_fraction;
		hout << "Ğ¡ÓÚÌå»ı·ÖÊı±ê×¼" << clust_geo.vol_fra_criterion << "£¬Çë¼ì²é" << endl;
		return 0;
	}
...
void GeoNano::Export_cluster_ellipsoids_mesh(const struct RVE_Geo &cell, const vector<struct elliparam> &ellips)const
{
	...
	for(int i=0; i<(int)ellips.size(); i++)
	{
		...
		const int num_phi = int(2*num_sita*ellips[i].a/ellips[i].c+0.5);  //ËÄÉáÎåÈëÈ¡Õû
...
void GeoNano::Export_cluster_ellipsoids_data(const vector<struct elliparam> &ellips, const double &ellip_ratio)const
{
	...
	out <<"%µ¥°ûÄÚÍÅ´ØÍÖÇòµÄ¸öÊıºÍÊµ¼ÊËùÕ¼Ìå»ı·ÖÊı" << endl;
	out << (int)ellips.size() << " " << ellip_ratio <<endl;
	out <<"%Êä³öÍÖÇò15¸ö²ÎÊı:ÍÖÇòÖĞĞÄ×ø±ê(x,y,z)¡¢°ëÖá³ß´ç(a,b,c)¡¢·½Î»½Ç(alpha1,beta1,gamma1; alpha2,beta2,gamma2; alpha3,beta3,gamma3)" << endl;
	for(int i=0; i<(int)ellips.size (); i++)
	{
		out	<< i << "  "
...
int GeoNano::Cnt_cross_cluster_surface(const double &growth_probability, const vector<struct elliparam> &ellips, int &ellip_num, int &seed, const Point_3D &point0, const Point_3D &point1)const
{
	...
	double x1, y1, z1;
	double f0, f1;
	if(ellip_num<-1||ellip_num>=(int)ellips.size()) { hout << "×¢Òâ£¡¼ÇÂ¼ÍÖÇò±àºÅ´íÎó£¬²»ÔÚËùÔÚ·¶Î§Ö®ÄÚ£¬Çë¼ì²é£¡" << endl; return 0; }
	if(ellip_num==-1)  //Ñ­»·ÍÖÇò
	{
		for(int i=0; i<(int)ellips.size(); i++)
		{
			...
			if(f0<=Zero) //ÔÚ´ËÍÖÇòÄÚ²¿
			{
				...
				if(f1>Zero)
				{
					...
					{
						ellip_num = i;
						return 1;	//ÄÉÃ×¹ÜÒªÔÚÍÅ´ØÄÚÕÛ»ØÉú³¤
					}
					else return 0; //¼ÌĞøÉú³¤
				}
				else return 0;	//¼ÌĞøÉú³¤
	...
	{
		//Õâ¶Î³ÌĞòÎªÁË¼ì²âÓëÍÖÇòÃæÏà½»µãµÄ¼ÆËã½á¹û£¬µ±³ÌĞò±àÊ±£¬´Ë¶Î³ÌĞò¿ÉÒÔ×¢ÊÍµô
		...
		if(f0>Zero) { hout << "´íÎó! ´Ëµã±¾À´Ó¦¸ÃÔÚÍÖÇò" << ellip_num << "ÄÚ²¿, µ«ÏÖÔÚ¼ÆËãÈ´ÔÚÍÖÇòÍâ²¿(f0>Zero), Çë¼ì²é£¡ " << f0 << endl; return 0; } //ÅĞ¶ÏÓï¾ä
	...
	return 0;  //Ã»ÓĞ´©Ô½ÍÅ´Ø±íÃæ£¬¼ÌĞøÉú³¤
}
//---------------------------------------------------------------------------
//ÅĞ¶ÏÄÉÃ×¹ÜÊÇ·ñ´©Ô½Ò»¸öÍÅ´ØÍÖÇò±íÃæ
...
{
	//--------------------------------------------------------------------------------------------------------
	//Õâ¶Î³ÌĞòÎªÁË¼ì²âÓëÍÖÇòÃæÏà½»µãµÄ¼ÆËã½á¹û
	...
	if(ef>Zero) { hout << "´íÎó! ´Ëµã±¾À´Ó¦¸ÃÔÚÍÖÇòÄÚ²¿, µ«ÏÖÔÚ¼ÆËãÈ´ÔÚÍÖÇòÍâ²¿(ef>Zero), Çë¼ì²é£¡ " << ef << endl; return 0; } //ÅĞ¶ÏÓï¾ä
...
int GeoNano::Get_intersection_cnt_ellipsoid(struct elliparam &temp_ellips, const Point_3D &point0, const Point_3D &point1, Point_3D &intersect_point)const
{
	...
	if(count!=1)
	{
		if(count==0&&t[0]>-1.0E-6)	{ intersect_point.flag = -1; return 1; }   //±íÃ÷½»µãÊµ¼ÊÉÏÔÚÏß¶ÎÒÔÍâ£¬Ô­ÒòÊÇÒòÎª0ºÅµãÔÚÍÖÇò½çÃæÇøÓò£¨½çÏŞ-1.0E-6¸ù¾İÒ»´Î³ö´íµÄĞÅÏ¢¸³ÓèµÄÖµ£©
		else { hout << "´íÎó£¡Ïß¶ÎÓëÍÖÇòÃæÃ»ÓĞ½»µã»òÕßÏß¶ÎÓëÍÖÇòÃæÍ¬Ê±ÓĞÁ½¸ö½»µã, Çë¼ì²é£¡" << " t[0]=" << t[0] << " t[1]=" << t[1] << endl; return 0; }
	}
	...
	intersect_point.flag = 1; //±íÃ÷Õâ¸öµãÊÇÕæÊµÕÒµ½µÄµã
	
	return 1;
}
//---------------------------------------------------------------------------
//ÒÆ¶¯ÄÉÃ×¹ÜÏß¶ÎÔÚÍÖÇòÃæÍâ¶Ëµãµ½ÍÖÇòÃæÄÚµÄ¶Ô³Æµã£¨ÒÔÇĞÃæÎª¶Ô³ÆÃæ£©
...
//---------------------------------------------------------------------------
//¼ÆËãÈı¸öµãÖĞÖĞ¼äÒ»µã´¦Ëù¼ĞµÄ¼Ğ½ÇµÄCosÖµ
...
int GeoNano::Get_seed_point(const struct RVE_Geo &cell, int &seed, Point_3D &point)const
{
	...
	point.flag = 0; //±êÃ÷ÊÇÄÉÃ×¹ÜµÄÆğÊ¼µã
...
{
	int cnt_seed_count =0; //¼ÇÂ¼ÈöÁË¶àÉÙ´ÎÄÉÃ×¹ÜÖÖ×Ó
	do
	{
		//-------------------------------------------------------------
		//Éú³ÉÖÖ×Ó
		...
		cnt_seed_count++;					//¼ÇÂ¼Ñ¡ÖÖÒ»´Î
		if(cnt_seed_count>100000) { hout << "ÄÉÃ×¹ÜÈöÖÖÔÚµ¥°ûÄÚµÄËùÓĞÍÅ´ØÍâÈöÖÖÊ®Íò´Î£¬ÈÔÃ»ÕÒµ½ºÏÊÊµÄÖÖ×Óµã£¡ Çë¼ì²é£¡" << endl; return 0; }
        
	}while(Judge_ellipses_including_point(ellips, point)>=0); //ÅĞ¶ÏÊÇ·ñÔÚÍÖÇòÖĞ,-1±íÊ¾ÔÚËùÓĞÍÖÇòÒÔÍâ
...
int GeoNano::Get_seed_point_inside_clusters(const struct elliparam ellip, int &seed, Point_3D &point)const
{
	...
	}while(pow(point.x,2)/pow(ellip.a, 2)+pow(point.y,2)/pow(ellip.b, 2)+pow(point.z,2)/pow(ellip.c, 2)-1.0>=-Zero);  //ÔÚÍÖÇòÍâ²¿ÒªÖØĞÂÉú³É
    
	//-------------------------------------------------------------
	//×ª»»µ½ÏÖÔÚµÄÍÖÇò×ø±êÏµÏÂ
...
int GeoNano::Get_uniform_direction(const struct CNT_Geo &cnts_geo, int &seed_sita, int &seed_pha, double &cnt_sita, double &cnt_pha)const
{
	...
	else if(cnts_geo.dir_dist_type=="specific")
	{
		...
		{
			cnt_sita = cnts_geo.ini_sita; //Õı·½Ïò
			cnt_pha = cnts_geo.ini_pha; //Õı·½Ïò
		}
		else
		{
			cnt_sita = PI - cnts_geo.ini_sita; //·´·½Ïò
			cnt_pha = PI + cnts_geo.ini_pha; //·´·½Ïò
		}
	}
...
{
	Point_3D temp_poi(point1.x-point0.x, point1.y-point0.y, point1.z-point0.z); //µãÏòÁ¿
...
//---------------------------------------------------------------------------
//ÅĞ¶ÏµãÊÇ·ñ±»°üº¬ÔÚÍÖÇòÖĞ(²¢·µ»ØÍÖÇòµÄ±àºÅ, Ç°ÌáÊÇÕâ×éÍÖÇò²¢²»Ïà½»)
...
int GeoNano::Get_intersecting_point_RVE_surface(const struct RVE_Geo &cell, Point_3D &point0, Point_3D &point1, vector<Point_3D> &ipoi_vec)const
{
	...
	for(int i=0; i<6; i++)
	{
		if(t_temp[i]>=0&&t_temp[i]<1)
		{
			...
			while(right>=left)
			{
				...
				if(fabs(t_ratio[middle] - t_temp[i])<Zero) goto T_Value_Same; //ÖµÏàÍ¬µÄÇé¿ö
				else if(t_ratio[middle] > t_temp[i]) right = middle - 1;
				else left = middle + 1;
			}
			t_ratio.insert(t_ratio.begin()+left, t_temp[i]);	//²åÈë
        T_Value_Same: ;
		}
	}
	...
	{
		hout << "´íÎó£¡ÄÉÃ×¹ÜÉú³¤Ïß¶ÎÓëRVE±íÃæµÄ½»µã¸öÊıÎª" << (int)t_ratio.size() << ", ÉÙÓÚÒ»¸ö»òÕß¶àÓÚÈı¸ö£¡ Çë¼ì²é£¡ " << endl;
		return 0;
	}
...
int GeoNano::Periodical_coordinate_transformation(const struct RVE_Geo &cell_geo, Point_3D point[])const
{
	...
	{
		hout << "´íÎó£¡ RVEÍâÒ»µãÓëRVE±íÃæÉÏÒ»µãÁ¬ÏßµÄÖĞµãÔÚRVEÄÚ²¿£¡Çë¼ì²é£¡ " << endl;
		return 0;
	}
	...
	{
		hout << "´íÎó£¡ ×ø±ê±ä»»ºóµÄµãÈÔÈ»²»ÔÚRVEÄÚ²¿£¡Çë¼ì²é£¡ " << endl;
		return 0;
	}
...
{
	//Éú³ÉÓÃÓÚ±íÊ¾ÄÉÃ×¹ÜµÄ½Úµã¼°ËÄÃæÌåÍø¸ñ
	...
	//Êä³öÄÉÃ×Ïß¼°(µ¥ÇøÓò)Í¿²ãÇøÓòÍø¸ñ(ÔÚº¯Êı(Export_cnt_threads_coating_mesh))ÖĞµ÷ÓÃ
    //	if(Export_cnts_threads_coating_singlezone(cell, cnts_nodes, cnts_eles, cnts_points)==0) return 0;
    
	//Êä³öÄÉÃ×Ïß¼°(¶àÇøÓò)Í¿²ãÇøÓòÍø¸ñ(ÔÚº¯Êı(Export_cnt_threads_coating_mesh))ÖĞµ÷ÓÃ
...
int GeoNano::Export_cnts_threads_coating_singlezone(const struct RVE_Geo &cell, const vector<vector<Node> > &nodes, const vector<vector<Element> > &eles, const vector<vector<Point_3D> > &cnts_points)const
{
	...
	if(cnts_account!=(int)eles.size()) { hout << "½ÚµãºÍµ¥ÔªÏÔÊ¾µÄÄÉÃ×¹ÜÏßÊıÁ¿²»Ò»ÖÂ£¡ Çë¼ì²é£¡" << endl; return 0; }
...
//---------------------------------------------------------------------------
//Êä³öÄÉÃ×¹ÜÍøÂç¼¸ºÎ¹¹Ôì(ËÄÃæÌåÍø¸ñTecplot)
                                                  ...
                                                  vector<vector<Element> > &coating_eles, const vector<vector<Point_3D> > &cnts_points, const double &coating_thickness)
{
	...
	for(int i=0; i<(int)cnts_points.size(); i++)
	{
		...
		for(int j=0; j<cps; j++)
		{
			...
			if(j==0)
			{
				double normal_sita, normal_pha;  //¼Ğ½Ç
				//µÃµ½ÏòÁ¿plane_normalÔÚÇò×ø±êÏµÖĞµÄ¼Ğ½Ç
				if(Get_angles_vector_in_spherial_coordinates(plane_normal, normal_sita, normal_pha)==0) return 0;
                
				//µÃµ½µã·¨Ê¸Æ½ÃæÉÏ(ÒÔµãÎªÖĞĞÄÒÔÒ»¶¨ÖµÎª°ë¾¶)Ò»¸öÔ²»·ÉÏµÄÒ»×éµã, ·¨Ê¸½Ç¶È(ÇòÃæ×ø±ê)ÒÑÖª
				if(Get_points_circle_in_plane(plane_center, normal_sita, normal_pha, cnts_radius[i], num_sec, cnts_nod_temp)==0) return 0;	//¼ÇÂ¼cnt±ß½çÃæÉÏµÄµã
				if(Get_points_circle_in_plane(plane_center, normal_sita, normal_pha, cnts_radius[i]+coating_thickness, num_sec, coating_nod_temp)==0) return 0;	//¼ÇÂ¼coating±ß½çÃæÉÏµÄµã
			}
			else
			{
				//¼ÆËãÇ°Ò»¸öÔ²ÉÏµÄµãÑØÇ°Ò»¸öÏß¶Î·½Ïò(line_vec)ÔÚ¹ıµãplane_center·¨Ê¸plane_normalµÄÆ½ÃæÉÏµÄÍ¶Ó°µã
				Point_3D line_vec;
				...
				if(Get_projected_points_in_plane(plane_center, plane_normal, line_vec, num_sec, cnts_nod_temp)==0) return 0;			//¼ÇÂ¼cnt±ß½çÃæÉÏµÄµã
				if(Get_projected_points_in_plane(plane_center, plane_normal, line_vec, num_sec, coating_nod_temp)==0) return 0;	//¼ÇÂ¼coating±ß½çÃæÉÏµÄµã
			}
			
			//²åÈëµ½Í¿²ã½ÚµãÏòÁ¿(È¥³ıÖĞĞÄµã)£¬½«ËùÓĞµÄµãµÄÀàĞÍ¸³Öµ
			...
			if(j!=0)
			{
				...
				//ÄÉÃ×¹ÜµÄµ¥Ôª
				int nodes_cnt[6];
				nodes_cnt[0] = (j-1)*(num_sec+1);   //ÖĞĞÄµã±àºÅ
...
int GeoNano::Generate_cnts_nodes_elements(vector<vector<Node> > &nodes, vector<vector<Element> > &eles, const vector<vector<Point_3D> > &cnts_points)
{
	...
	for(int i=0; i<(int)cnts_points.size(); i++)
	{
		...
		for(int j=0; j<cps; j++)
		{
			...
			{
				int nodes_num[6];
				nodes_num[0] = (j-1)*(num_sec+1);   //ÖĞĞÄµã±àºÅ
...
//---------------------------------------------------------------------------
//µÃµ½ÏòÁ¿ÔÚÇò×ø±êÏµÖĞµÄ¼Ğ½Ç
int GeoNano::Get_angles_vector_in_spherial_coordinates(const Point_3D &normal, double &sita, double &pha)const
{
	if(normal.x==0&&normal.y==0&&normal.z==0) { hout << "ÏòÁ¿µÄÈı¸ö·ÖÁ¿¶¼ÎªÁã£¡ Çë¼ì²é£¡" << endl; return 0; }
...
int GeoNano::Get_projected_points_in_plane(const Point_3D &center, const Point_3D &normal, const Point_3D &line, const int &num_sec, vector<Node> &nod_temp)const
{
	...
	{
		hout << "Á½¸ö·¨ÏòÁ¿³ÉÖ±½Ç(¶ÔÓ¦Èı¸öµã0¡¢1¡¢2, Ö»ÓĞµ±½Ç012Ğ¡ÓÚ90¶È£¬ÇÒÏß¶Î12>Ïß¶Î01Ê±, ²Å¿ÉÄÜ³öÏÖÕâÖÖÇé¿ö£¬¶øÕâ»ù±¾ÉÏÊÇ²»¿ÉÄÜµÄ)£¡ Çë¼ì²é£¡" << endl;
		return 0;
	}
                                       ...
                                       const vector<vector<Node> > &coating_nodes, const vector<vector<Element> > &coating_eles)const
{
	...
	if(cnts_account!=(int)cnts_eles.size()) { hout << "½ÚµãºÍµ¥ÔªÏÔÊ¾µÄÄÉÃ×¹ÜÏßÊıÁ¿²»Ò»ÖÂ£¡ Çë¼ì²é£¡" << endl; return 0; }
	const int coating_account = (int)coating_nodes.size();
	if(coating_account!=(int)coating_eles.size()) { hout << "½ÚµãºÍµ¥ÔªÏÔÊ¾µÄÍ¿²ãÊıÁ¿²»Ò»ÖÂ£¡ Çë¼ì²é£¡" << endl; return 0; }
...
int GeoNano::Export_cnts_discerned_clusters(const struct RVE_Geo &cell, const vector<vector<Node> > &nodes, const vector<vector<Element> > &eles, const vector<struct elliparam> &ellips)const
{
	...
	//---------------------------------------------------------------------------
	//¼ÆËã²¢·Ö±ğ´æ´¢ÔÚÍÖÇòÄÚ»¹ÊÇÍâµÄÄÉÃ×¹ÜÍø¸ñµ¥Ôª
	vector<Element> ele_clus[2];  //·Ö±ğ´æ´¢ÔÚÍÖÇòÄÚ»òÍâµÄÄÉÃ×¹ÜÍø¸ñµ¥Ôª
	Point_3D poi_clus;					 //Íø¸ñµ¥ÔªµÄĞÎĞÄµãÎ»ÖÃ
	int nodes_num = 0;					 //¼ÇÂ¼µ¥Ôª½ÚµãµÄÊµ¼ÊÎ»ÖÃ
	for(int i=0; i<cnts_account; i++)
	{
		if(i!=0) nodes_num += (int)nodes[i-1].size();
		for(int j=0; j<(int)eles[i].size(); j++)
		{
			...
			for(int k=0; k<4; k++)
			{
				ele_temp.nodes_id.push_back(eles[i][j].nodes_id[k] + nodes_num);  //¼ÇÂ¼´Ëµ¥Ôª
			...
			else { hout << "Ò»¸öµãÍ¬Ê±ÔÚ" << key << "¸öÍÖÇòÄÚ²¿£¡ Çë¼ì²é£¡" << endl; return 0; }
		}
	}
...
int GeoNano::Estimate_cnt_cluster_parameters(const struct RVE_Geo &cell, const vector<vector<Point_3D> > &cnts_points, const vector<struct elliparam> &ellips, const struct CNT_Geo &cnts_geo, struct Clust_Geo &clust_geo)const
{
	...
	if(clust_geo.vol_fra_criterion!=0&&(int)ellips.size()>0)
	{
		...
		for(int i=0; i<(int)cnts_points.size(); i++)
			for(int j=0; j<(int)cnts_points[i].size()-1; j++)
			{
				...
				total_cnt_length += temp_len;		//ÄÉÃ×¹ÜµÄ×Ü³¤¶È
                
				double temp_vol = temp_len*cnts_radius[i]*cnts_radius[i]*PI;
				total_cnt_volume += temp_vol;		//ÄÉÃ×¹ÜµÄ×ÜÌå»ı
				...
				for(int k=0; k<(int)ellips.size(); k++)
				{
					...
					if(f0<=Zero) 
					{ 
						cluster_cnt_length += temp_len;				 //ÍÅ´ØÍÖÇòÖĞÄÉÃ×¹ÜµÄ³¤¶È
						clust_geo.cnt_real_volume += temp_vol;   //ÍÅ´ØÍÖÇòÖĞÄÉÃ×¹ÜµÄÌå»ı
						break; 
					}
				}
			}
		
		//ÍÅ´ØÍÖÇòÖĞÄÉÃ×¹ÜµÄÖØÁ¿
	...
	if(cnts_geo.criterion=="vol")
	{
		hout << "    ÄÉÃ×¹ÜÍÅ´ØÍÖÇòÖĞÄÉÃ×¹ÜËùÕ¼Ìå»ı·ÖÊı£º" << clust_geo.cnt_real_volume << endl;
		hout << "    µ¥°ûÄÚËùÓĞÄÉÃ×¹ÜËùÕ¼Ìå»ı·ÖÊı£º" << total_cnt_volume << endl;
	}
	else if(cnts_geo.criterion=="wt"||cnts_geo.criterion=="nwt")
	{
		hout << "    ÄÉÃ×¹ÜÍÅ´ØÍÖÇòÖĞÄÉÃ×¹ÜËùÕ¼±ÈÖØ£º" << clust_geo.cnt_real_weight/(cell_geo.volume*cell_geo.density) << endl;
		hout << "    µ¥°ûÄÚËùÓĞÄÉÃ×¹ÜËùÕ¼±ÈÖØ£º" << total_cnt_weight/(cell_geo.volume*cell_geo.density) << endl;
		hout << "    ÍÅ´ØÍÖÇòÖĞÄÉÃ×¹ÜÕ¼µ¥°ûÄÚËùÓĞÄÉÃ×¹Ü±ÈÀı£º" << clust_geo.cnt_real_weight/total_cnt_weight << endl;
	}
    
	return 1;
}
//---------------------------------------------------------------------------
//¼ÇÂ¼ÄÉÃ×¹ÜµãĞÅÏ¢(CNTs pointsµÄflag==0±íÊ¾ÄÉÃ×¹ÜµÄÆğÊ¼µã; flag>0±íÊ¾ÄÉÃ×¹ÜÄÚµÄµã, °´ĞòÒÀ´Î±àºÅ)
int GeoNano::Record_cnt_points_information(const vector<vector<Point_3D> > &cnts_points)
{
    ...
    ct_end = clock();
    hout << "Record_cnt_points_information time: "<<(double)(ct_end-ct_begin)/CLOCKS_PER_SEC<<"Ãë¡£"<<endl;
    return 1;
}
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
