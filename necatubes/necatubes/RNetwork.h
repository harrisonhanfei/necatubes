...
public:
    ...
    int Locate_and_trim_boundary_cnts(string update, vector<Point_3D> &points_in,vector<vector<long int> > &structure, vector<double> &radii);
    ...
    int Update_region_of_interest(vector<Point_3D> &points_in, struct Region_Geo cnt_regions, vector<vector<long int> > &structure, vector<vector<int> > sectioned_domain_cnt, vector<double> &radii);
    int Update_contacts(long int boundary_l, int CNT, vector<Point_3D> &points_in, vector<vector<long int> > structure);
    int Delete_cnt_and_point_contacts(long int point, int CNT, vector<Point_3D> points_in);
    ...
    int Analysis(const struct RVE_Geo cell_geo, struct CNT_Geo cnts_geo, vector<Point_3D> points_in, vector<double> cnts_radius, vector<vector<long int> > structure);
    int Make_CNT_clusters(struct CNT_Geo cnts_geo, vector<Point_3D> points_in, vector<vector<long int> > structure);
    void Clear_vectors();
    int Single_cluster(int cnt_seed, vector<int> &vec, vector<vector<int> > &contacts_vector);
    ...
    void Fill_stiffness_matrix(vector<int> cluster, vector<vector<long int> > structure, vector<int> LM_matrix, MathMatrix &stiffness);
    void Add_to_stiffness(long int node1, long int node2, MathMatrix &stiffness);
    ...
    MathMatrix Solve_DEA_equations_CG_SSS(MathMatrix stiffness);
